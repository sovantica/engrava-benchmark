...
This is a public repository (engrava-benchmark) — Public reproduction surface for Engrava memory-benchmark numbers. License: MIT. Primary language: python 3.11+.
...
A commit-message hook is installed locally to give you a fast feedback loop on commit-message hygiene. Run `git commit` and it fires on the patterns it checks for. It is a pattern check, not a judge of whether a message reads well for a public project - what it does not check is still yours and the reviewer's to get right.

## Branch hygiene

Branch names: `<type>/<kebab-description>`. Plain English. No internal identifiers. ≤ 50 chars.

Benchmark result rows use `results/<kebab-description>` branches and `chore(results): ...` commit messages.

Examples:
- `feature/priority-signal-hybrid-search`
- `results/engrava-050-default-run`
...
When uncertain, default to **omitting the reference**. The local commit-message hook will tell you when it sees something it doesn't like — read its output and adjust.
